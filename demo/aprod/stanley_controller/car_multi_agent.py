...
class CarAgent(BaseAgent):
    # ep_d = 0.5 # use these to enforce correct sampling -- not sure this is the right approach, but try it anyways
    # ep_psi = np.pi/18
    ...
    def __init__(
        ...
        speed: float = 2,
        accel: float = 1,
        ep_d: float = 0, # standard noise is 0.5 
        ep_psi: float = 0, # standard noise if np.pi/18 or 10 degrees
    ):
        super().__init__(
    ...
    @staticmethod
    def dynamics(t, state, u):
        x, y, theta, v, d, heading, timer, priority = state # the d and psi are possibly noisy
        delta, a = u
        x_dot = v * np.cos(theta + delta)
        y_dot = v * np.sin(theta + delta)
        theta_dot = v / 1.75 * np.sin(delta)
    ...
    def action_handler(self, mode: List[str], state, lane_map: LaneMap) -> Tuple[float, float]:
        ...
        #     """
        #     Returns the clipped version of sample_x based on the true x and ep_x linear error bounds 
        #     """
        #     if sample_x > x+ep_x:
        #         return x+ep_x
        #     if sample_x < x-ep_x:
        ...
        x, y, theta, v, d_sample, heading_sample, timer, priority = state
        vehicle_mode, vehicle_lane, sensor_mode = mode # NOTE: doubt this is necessary
        vehicle_pos = np.array([x, y])
        a = 0

        d = -d_sample # NOTE: need to invert because of convention, see original car_agent file
