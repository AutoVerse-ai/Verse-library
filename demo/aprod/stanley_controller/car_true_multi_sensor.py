...
from verse.analysis.parsed_wrap import get_heading_bounds_optimized, get_lateral_distance_bounds_optimized, get_long_pos_bounds_optimized
        ...
        if simulate: # just start with simulation 
                ...
                if cur_agent == agent.id:
                    ...
                    for other_agent in state_dict:
                        if other_agent == cur_agent:
                            continue
                        other_priority, other_sensor_mode = state_dict[other_agent][0][8], state_dict[other_agent][1][2] # NOTE: assume these exist, should for the most part 
                    ...
                    cont['ego.d'] = d+np.random.uniform(-1, 1)*self.ep_d
                    cont['ego.psi'] = psi+np.random.uniform(-1, 1)*self.ep_psi
                    
                    if not 'npc' in cur_agent: 
                        long = lane_map.get_longitudinal_position(disc['ego.track_mode'], pos)
                        cont['extra.long'] = long
                    ...
                    if 'npc' not in agent.id:
                        other_x = state_dict[cur_agent][0][1]
                        other_y = state_dict[cur_agent][0][2]
            ...
            for cur_agent in state_dict:
                ...
                if cur_agent == agent.id:
                    ...
                    if not 'npc' in cur_agent: 
                        long_min, long_max = get_long_pos_bounds_optimized(lane_map, disc['ego.track_mode'], cont['ego.x'], cont['ego.y'])
                        cont['extra.long'] = [long_min, long_max]
                        
                        # print(f'Lat dist: {cont["ego.d"]}')

                    if has_priority:
                        cont['extra.sensor_update'] = [1, 1]
                    else:
                        cont['extra.sensor_update'] = [0,0]

                else: # NOTE: not sure this is necessary, all relevant computations are occur in main 'if' block
                    if 'npc' not in agent.id:
                        other_x = [state_dict[cur_agent][0][0][1], state_dict[cur_agent][0][1][1]]
                        other_y = [state_dict[cur_agent][0][0][2], state_dict[cur_agent][0][1][2]] 

                        other_track_mode = state_dict[cur_agent][1][1]
                        other_long_min, other_long_max = get_long_pos_bounds_optimized(lane_map, other_track_mode, other_x, other_y)
                        if 'others.x' not in cont: # for now just consider perfect other info scenario
                            cont['others.long'] = [[other_long_min, other_long_max]]

                            disc['others.track_mode'] = [state_dict[cur_agent][1][1]]
                        else:
                            cont['others.long'].append([other_long_min, other_long_max])
