...
class VehiclePedestrianSensor:
    ...
    def sense(self, agent: BaseAgent, state_dict, lane_map, simulate = False):
        len_dict = {}
        cont = {}
        disc = {}
...
class VehiclePedestrianSensorStar:
    def __init__(self):
        self.sensor_distance = 60
    ...
    def sense(self, agent: BaseAgent, state_dict, lane_map, simulate = False):
        ...
        if agent.id == 'car':
            ...
            lb11, ub11 = state_dict['car'][0][1].get_max_min(0)

            lb12, ub12 = state_dict['car'][0][1].get_max_min(1)

            lb21, ub21 = state_dict['pedestrian'][0][1].get_max_min(0)

            lb22, ub22 = state_dict['pedestrian'][0][1].get_max_min(1)
            ...
            lateral_set = from_rect([dist_min],[dist_max])

            starset = state_dict['car'][0][1]
            #print(starset.basis, starset.center)
            #starset.center[-1] = (dist_max+dist_min)/2
            #starset.basis[4,-1] = starset.center[-1] - dist_min
            cont['ego.x'] = starset
            
            cont['ego.y'] = starset
        
            cont['ego.theta'] = starset
            
            cont['ego.v'] = starset
            
            cont['other.dist'] = lateral_set

            
            disc['ego.agent_mode'] = state_dict['car'][1][0]
            if dist_min>self.sensor_distance:
                cont['other.dist'] = from_rect([1000], [1000])
                #starset.center[-1] = 1000
                #starset.basis[4,-1] = 0
                # cont['other.x'] = [
                #     1000, 1000
                # ]
