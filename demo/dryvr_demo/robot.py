...
if __name__ == "__main__":
    ...
    initial_set_polytope = pc.box2poly([[1.5,1.51],[1.5,1.51],[0,0.01],[0,0.01]])
    Robot.set_initial(StarSet.from_polytope(initial_set_polytope), (RobotMode.Normal,))

    scenario.config.reachability_method = ReachabilityMethod.STAR_SETS
    scenario.add_agent(Robot)
    ...
    print(run_time)
    print("diams")
    diams = time_step_diameter(trace, 10, 0.01)
    print(len(diams))
    print(sum(diams))
    print(diams[0])
    print(diams[-1])
