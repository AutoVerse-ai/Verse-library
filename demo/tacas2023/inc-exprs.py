...
for xprm in xprms:
    if compare:
        cmd = Popen(
            f"gtime --verbose -- python3 demo/tacas2023/exp11/inc-expr.py c{xprm} {'i' if 'i' in xprm else ''}l",
            stdout=PIPE,
            stderr=PIPE,
            shell=True,
        )
        ...
        cmd = Popen(
            f"gtime --verbose -- python3 demo/tacas2023/exp11/inc-expr.py {xprm}{'l' if pal else ''}",
            stdout=PIPE,
            stderr=PIPE,
            shell=True,
        )
    ...
    if len(filtered_info) == 0:
        ...
        if info["# Tr"] != info2["# Tr"]:
            ...
            rslt = ExperimentResult(
                xprm,
                max_mem,
                [info["Run Time"], info2["Run Time"]],
                [info["cache_size"], info2["cache_size"]],
                info["# Tr"],
                ret,
                [info["cache_hit"], info2["cache_hit"]],
            )
            rslts.append(rslt)
    pp(rslt)
    if rslt.ret_code != 0:
        ...
        print(
            ...
            + " & ".join(
                ...
                + [
                    ...
                    for i in [
                        inc.node_count,
                        round(inc.duration[0], 2),
                        round(inc.duration[1], 2),
                        round(inc.cache_size[0], 2),
                        round(inc.cache_size[1], 2),
                        round(no_cache_hit_rate * 100, 2),
                        round(par_cache_hit_rate * 100, 2),
                    ]
                ]
            )
            + " \\\\"
