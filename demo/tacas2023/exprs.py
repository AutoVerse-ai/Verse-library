...
for expr in expr_list:
    ...
    if len(filtered_info) == 0:
        ...
        if info['# Tr'] != info2['# Tr']:
            rslt = ExperimentResult(expr, max_mem, ret, info["#A"], info["A"], info['Map'], info['postCont'], info['Noisy S'], info['# Tr'], info['Run Time'], None, None)
            rslts.append(rslt)
            rslt = ExperimentResult(expr, max_mem, ret, info2["#A"], info2["A"], info2['Map'], info2['postCont'], info2['Noisy S'], info2['# Tr'], info2['Run Time'], None, None)
            rslts.append(rslt)
        else:
            equal = [l.decode("utf-8") for l in stdout if b"? True" in l]
            rslt = ExperimentResult(expr, max_mem, ret, info["#A"], info["A"], info['Map'], info['postCont'], info['Noisy S'], info['# Tr'], info['Run Time'], info2['Run Time'], len(equal) == 2)
            rslts.append(rslt)            
    pp(rslt)
    if ret != 0:
...
for res in rslts:
    if res.duration2 is None:
        ...
        print(f"{res.num_A} & {res.type_A} & {res.Map} & {res.postCont} & {res.noisy_S} & {res.node_count} & {round(res.duration,2)} & {round(res.duration2,2)} & {res.equality}\\\\")
