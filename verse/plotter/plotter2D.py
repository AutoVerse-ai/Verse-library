...
def simulation_tree(root: Union[AnalysisTree, AnalysisTreeNode], map=None, fig=go.Figure(), x_dim: int = 1, y_dim: int = 2, print_dim_list=None, map_type='lines', scale_type='trace', label_mode='None', sample_rate=1):
    ...
    for agent_id in agent_list:
        # replace schemelist[i] with None

        fig = simulation_tree_single(root, agent_id, fig, x_dim, y_dim, None, print_dim_list)
        i = (i+1) % num_theme
    if scale_type == 'trace':
...
def reachtube_tree(root: Union[AnalysisTree, AnalysisTreeNode], map=None, fig=go.Figure(), x_dim: int = 1, y_dim: int = 2, print_dim_list=None, map_type='lines', scale_type='trace', label_mode='None', sample_rate=1, combine_rect=1, plot_color = None):
    ...
    for agent_id in agent_list:
        fig = reachtube_tree_single(
            root, agent_id, fig, x_dim, y_dim, None, print_dim_list, combine_rect, plot_color=plot_color)
        i = (i+1) % num_theme
    if scale_type == 'trace':
        queue = [root]
        x_min, x_max = float('inf'), -float('inf')
        ...
        for agent_id in traces:
            ...
            if label_mode != 'None':
                ...
                if node.assert_hits != None and agent_id in node.assert_hits[0]:
                    fig.add_trace(go.Scatter(x=[trace[-1, x_dim]], y=[trace[-1, y_dim]],#[ list(np.array(trace[-1, 1]) + np.array(trace[-1, 2]) +  np.array(trace[-1,3])) ],
                                            mode='markers+text',
                                            text=['HIT:\n' +
        ...
        if len(np.unique(np.array([trace[i][x_dim] for i in range(0, max_id)]))) == 1 and len(np.unique(np.array([trace[i][y_dim] for i in range(0, max_id)]))) == 1:
            ...
            for idx in range(0, len(trace), combine_rect*2):
                ...
                if max_id <= 2:
                    ...
                    # trace_y = np.array([
                    #     trace_seg[0][1] + trace_seg[0][2] + trace_seg[0][3] ,
                    #     trace_seg[0][1] + trace_seg[0][2] + trace_seg[0][3],
                    #     trace_seg[1][1] + trace_seg[0][2] + trace_seg[0][3],
                    #     trace_seg[1][1] + trace_seg[0][2] + trace_seg[0][3],
                    #     trace_seg[0][1] + trace_seg[0][2] + trace_seg[0][3],
                    # ])
                    ...
                    trace_y_odd = np.array(
                        ...
                        #[trace_seg[i][1]+ trace_seg[i][2] + trace_seg[i][3] for i in range(0, max_id, 2)])
                    trace_y_even = np.array(
                        [trace_seg[i][y_dim] for i in range(1, max_id + 1, 2)])

                        #[trace_seg[i][1]+ trace_seg[i][2] + trace_seg[i][3] for i in range(1, max_id+1, 2)])
