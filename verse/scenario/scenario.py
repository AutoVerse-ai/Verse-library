...
def convertStrToEnum(inp, agent:BaseAgent, dl):
    ...
    for field in res.__dict__:
        for state_def_name in agent.controller.state_defs:
            if field in agent.controller.state_defs[state_def_name].disc:
                idx = agent.controller.state_defs[state_def_name].disc.index(field)
                field_type = agent.controller.state_defs[state_def_name].disc_type[idx]
...
def disc_field(field:str, agent:BaseAgent):
    for state_def_name in agent.controller.state_defs:
        state_def = agent.controller.state_defs[state_def_name]
    ...
    def get_transition_simulate_simple(self, node: AnalysisTreeNode) -> Tuple[Optional[Dict[str, List[str]]], Optional[Dict[str, List[Tuple[str, List[str], List[float]]]]], int]:
        ...
        for idx in range(trace_length):
            ...
            for agent_id in agent_guard_dict:
                ...
                exec(agent.controller.controller_code,dl.__dict__)
