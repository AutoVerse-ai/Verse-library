...
class StraightLane_3d(AbstractLane_3d):
    ...
    def __init__(
        ...
        self.line_types = line_types or [LineType_3d.STRIPED, LineType_3d.STRIPED]
    ...
    def get_sample_points(self, num_theta, num_len):
        ...
        x = start[0] + ls * n[0] + r * (a[0] * np.cos(thetas) + b[0] * np.sin(thetas))
        y = start[1] + ls * n[1] + r * (a[1] * np.cos(thetas) + b[1] * np.sin(thetas))
        z = start[2] + ls * n[2] + r * (a[2] * np.cos(thetas) + b[2] * np.sin(thetas))
        return x, y, z
...
class CircularLane_3d_v1(AbstractLane_3d):
    ...
    def local_coordinates(self, position: np.ndarray) -> Tuple[float, float, float]:
        ...
        if phaseget > self.start_phase and phaseget > self.end_phase:
            phaseget -= 2 * pi
        lget = (phaseget - self.start_phase) * self.length / (self.end_phase - self.start_phase)
        return lget, rget, thetaget

    def get_outer_center(self, longitudinal: float):
        phase = self.start_phase + (self.end_phase - self.start_phase) * longitudinal / self.length
    ...
    def get_sample_points(self, num_theta, num_len):
        ...
        if self.start_phase <= self.end_phase and self.right_rotate:
            ...
            phases = np.mgrid[self.end_phase : (self.start_phase + 2 * pi) : num_len * 1j]
        elif self.start_phase >= self.end_phase and self.right_rotate:
            phases = np.mgrid[self.start_phase : (self.end_phase + 2 * pi) : num_len * 1j]
        elif self.start_phase >= self.end_phase and not self.right_rotate:
        ...
        oc_out_x = np.atleast_2d(oc_x) + (r_out * (a_out_x * costheta + b_out_x * sintheta))
        oc_out_y = np.atleast_2d(oc_y) + (r_out * (a_out_y * costheta + b_out_y * sintheta))
        oc_out_z = np.atleast_2d(oc_z) + (r_out * (a_out_z * costheta + b_out_z * sintheta))
...
class CircularLane_3d_v2(CircularLane_3d_v1):
    def __init__(
        ...
        center, radius = get_center_radius(start, end, norm_vec, delta_phase, right_rotate)
        start_phase = get_theta_by_coor(start, norm_vec, center, radius, right_rotate)
        end_phase = get_theta_by_coor(end, norm_vec, center, radius, right_rotate)
