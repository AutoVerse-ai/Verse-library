...
def check_valid_side(side):
    ...
    if side is not None:  # we want to check is the side we are traversing is NOT NONE
        ...
        for tl in temp_lanes:  # traverse each element in the left lane
            lane_array.append(
                tl["type"]
            )  # add each of the left segment to the temporary left array
...
def condense_matrix(drive_way_matrix, width_2d):
    ...
    for i in range(cols):
        lane_to_append = Lane(
            "T" + str(i), drive_way_matrix[:, i].tolist()
        )  # combine all the rows into one list for each column
...
def road_traverser(road):
    ...
    for elem in road:  # traverse each road segment in the open drive file
        ...
        left_valid, left_array, left_alpha_array = check_valid_side(
            left
        )  # check the left side of the lane

        #################RIGHT SIDE OF THE ROAD################################################################
        right = lane_types.find("right")  # find all the center side of the road
        right_valid, right_array, right_alpha_array = check_valid_side(
            right
        ...
        for rg in road_geom:  # traverse each geometry for each road
            ...
            if rg.line:  # we want to check if this is the line segment
                ...
                N_left = np.array([-delta_y, delta_x]) / (
                    np.sqrt(delta_x**2 + delta_y**2)
                )  # Normal Vector for left side
                N_right = np.array([delta_y, -delta_x]) / (
                    np.sqrt(delta_x**2 + delta_y**2)
                )  # Normal Vector for right side
                ##############################################################################
                ...
                if left_valid:
                    for left_elem in left_array:
                        if left_elem == "sidewalk":
                            ...
                            line_types = style
                            straight_to_append = StraightLane(
                                id,
                                left_new_vect_i,
                                left_new_vect_f,
                                width,
                                ...
                                20,
                                0,
                            )  # Straight (id, start vector, end vector, width, line type,forbidden,speed limit, priority)
                ...
                if curvature < 0:  # if the curvature is negative then it it CLOCKWISE
                    center = (
                        np.array([xi, yi]) - S[0:2] * radius
                    )  # center is equal to the current point subtract the radius
                    curvature_direction.append(False)
                else:  # if the curvature is positive then it is COUNTER-CLOCKWISE
                    center = (
                        np.array([xi, yi]) + S[0:2] * radius
                    )  # center is equal to the current point add the radius
                    curvature_direction.append(True)

                x_diff = (
                    xi - center[0]
                )  # x_diff means the difference between the current x-coordinate and the center x-coordinate
                y_diff = (
                    yi - center[1]
                )  # y_diff means the difference between the current y-coordinate and the center y-coordinate
                initial_theta = np.arctan2(
                    y_diff, x_diff
                )  # initial theta is the angle between the y_diff and x_diff
                delta_theta = abs(
                    arc_length / radius
                )  # delta theta is the final angle of the curvature

                if curvature < 0:  # if the curvature is negative
                    final_theta = (
                        initial_theta - delta_theta
                    )  # we want to go the opposite way so subtract delta theta from initial theta
                else:  # this condition means that the curvvature is positive
                    final_theta = (
                        initial_theta + delta_theta
                    )  # we want to go COUNTER-CLOCKWISE so ADD the delta theta to initial theta
...
def opendrive_map(
    ...
    left_lanes, left_widths = condense_matrix(
        left_drive_way_2d, left_width_2d
    )  # condense matrix returns left side of the road along the width 2d matrix
    right_lanes, right_widths = condense_matrix(
        right_drive_way_2d, right_width_2d
    )  # condense matrix returns the right side of the road along the width 2d matrix
    # right_lanes = temp_right() #just a temporary function to generate the right side of the road

    total = (
        left_lanes[::-1] + right_lanes
    )  # reverse the left lane order and then combine with the right lane list to create one long lane list
    completed_lanes = LaneMap(total)  # create a lane map

    for i in range(len(total) - 1, 0, -1):
        completed_lanes.h_dict[
            (total[i].id, "Normal", "SwitchLeft")
        ] = f"M{total[i].id[-1]}{total[i-1].id[-1]}"
        completed_lanes.h_dict[
            (f"M{total[i].id[-1]}{total[i-1].id[-1]}", "SwitchLeft", "Normal")
        ] = total[i - 1].id
        completed_lanes.left_lane_dict[total[i].id] = [
            total[i - 1].id
        ]  # connect each left side for each lane

    for i in range(len(total) - 1):
        completed_lanes.h_dict[
            (total[i].id, "Normal", "SwitchRight")
        ] = f"M{total[i].id[-1]}{total[i+1].id[-1]}"
        completed_lanes.h_dict[
            (f"M{total[i].id[-1]}{total[i+1].id[-1]}", "SwitchRight", "Normal")
        ] = total[i + 1].id

        completed_lanes.right_lane_dict[total[i].id] = [
            total[i + 1].id
        ]  # connect each right side for each lane
