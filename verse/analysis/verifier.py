...
class Verifier:
    ...
    def compute_full_reachtube_step_partitioned(
        ...
        if all_possible_transitions:
            for transition in all_possible_transitions:
                ...
                # Partition the hitting rect
                partitions = Verifier._partition_rect(hitting_rect, n)
    ...
    @staticmethod
    def _partition_rect(rect, n):
        # Partition a rectangle [lb, ub] into n^k sub-rectangles, where k is the number of varying dimensions
        lb, ub = rect
        d = len(lb)
        # Identify varying dimensions (where lb != ub)
        varying_dims = [i for i in range(d) if lb[i] != ub[i]]
        k = len(varying_dims)
        if k == 0:
            # All dimensions are singletons: return the original rect as one partition
            return [rect]
