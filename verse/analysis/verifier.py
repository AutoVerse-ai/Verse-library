...
class Verifier:
    ...
    def check_assert_guard_step(idx, end_idx, combine_len, state_dict:Dict, trans_const: TransConsts):
        asserts = defaultdict(list)
        hits = []
        reduction_needed = False
        any_contained = False
        for agent_id, agent in trans_const.agent_dict.items():
            ...
            cont_vars, disc_vars, len_dict = trans_const.sensor.sense(agent, state_dict, trans_const.track_map)
            ...
            for i, a in enumerate(agent.decision_logic.asserts_veri):
                ...
                def eval_expr(expr):
                    ...
                    sat = ge.evaluate_guard_disc(agent, disc_vars, cont_vars, trans_const.track_map)
                    if sat:
                        sat = ge.evaluate_guard_hybrid(agent, disc_vars, cont_vars, trans_const.track_map)
                        if sat:
                            sat, contained = ge.evaluate_guard_cont(agent, cont_vars, trans_const.track_map)
            ...
            if agent_id in asserts:
                continue
            if agent_id not in trans_const.agent_guard_dict:
                continue
        
            unchecked_cache_guards = [g[:-1] for g in trans_const.cached_guards[agent_id] if g[-1] < idx]     # FIXME: off by 1?
            for guard_expression, continuous_variable_updater, discrete_variable_dict, path in trans_const.agent_guard_dict[agent_id] + unchecked_cache_guards:
                ...
                guard_can_satisfied = one_step_guard.evaluate_guard_hybrid(
                    agent, discrete_variable_dict, new_cont_var_dict, trans_const.track_map)
                if not guard_can_satisfied:
                    continue
                guard_satisfied, is_contained = one_step_guard.evaluate_guard_cont(
                    agent, new_cont_var_dict, trans_const.track_map)
        ...
        #     return True, asserts, hits, any_contained
        # if len(asserts) > 0:
        #     return False, asserts, hits, any_contained
        # if hits != []:
        #     return False, asserts, hits, any_contained
        ...
        #     return
        # if any_contained:
        #     return False, asserts, hits, True
        return reduction_needed, asserts, hits, any_contained

    @staticmethod
    def get_transition_verify_opt_par(cache: Dict[str, CachedRTTrans], paths: PathDiffs, node: AnalysisTreeNode, track_map, sensor
        ...
        trans_const = TransConsts(agent_dict, sensor, track_map, agent_guard_dict, cached_guards)
        while reduction_queue:
            ...
            reduction_needed, asserts, hits, any_contained = Verifier.check_assert_guard_step(idx, end_idx, combine_len, state_dict, trans_const)
            if reduction_needed:
                ...
                reduction_queue.extend(next_list[::-1]) # DFS
                continue
            else:
                if len(asserts) > 0:
                ...
                if any_contained:
                    break
            # print('step check:', time.time()-start)
