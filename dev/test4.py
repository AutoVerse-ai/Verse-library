...
def reachtube_origin():
    # Decomposition for the original system  
    ...
    dt = 0.01
    expr = [
        x1 + dt*(x1-2*x2+x2**3+w1),
        x2 + dt*(x1-2*x2),
    ]
    # expr_func = lambdify([(x1,x2,w1)], expr)
...
def reachtube_transform1():
    ...
    tube = np.array(tube)
    tube[:,1:3] = tube[:,1:3]@np.array([[1,1],[0,1]]).T
    tube[:,3:5] = tube[:,3:5]@np.array([[1,1],[0,1]]).T
    return tube

def reachtube_transform2():
    # Decomposition for the original system  
    y1 = Symbol('y1',real=True)
    y2 = Symbol('y2',real=True)
    w1 = Symbol('w1', real=True)
    ...
    tube = np.array(tube)
    tube[:,1:3] = tube[:,1:3]@np.array([[1,2],[0,1]]).T
    tube[:,3:5] = tube[:,3:5]@np.array([[1,2],[0,1]]).T
    return tube

if __name__ == "__main__":
    trace1 = reachtube_origin()
    trace2 = reachtube_transform1()
    trace3 = reachtube_transform2()

    plt.figure(0)
    plt.plot(trace1[:,0], trace1[:,1],'r')
    plt.plot(trace2[:,0], trace2[:,1],'g')
    plt.plot(trace3[:,0], trace3[:,1],'b')

    plt.plot(trace1[:,0], trace1[:,1+2],'r')
    plt.plot(trace2[:,0], trace2[:,1+2],'g')
    plt.plot(trace3[:,0], trace3[:,1+2],'b')

    plt.figure(1)
    plt.plot(trace1[:,0], trace1[:,2],'r')
    plt.plot(trace2[:,0], trace2[:,2],'g')
    plt.plot(trace3[:,0], trace3[:,2],'b')

    plt.plot(trace1[:,0], trace1[:,2+2],'r')
    plt.plot(trace2[:,0], trace2[:,2+2],'g')
    plt.plot(trace3[:,0], trace3[:,2+2],'b')
