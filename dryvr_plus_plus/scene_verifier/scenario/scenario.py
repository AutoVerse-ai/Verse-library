...
class Scenario:
    ...
    def check_guard_hit(self, state_dict):
        ...
        for agent_id in state_dict:
            ...
            for path in paths:
                ...
                continuous_variable_dict, discrete_variable_dict, length_dict = self.sensor.sense(
                    self, agent, state_dict, self.map)

                # Unroll all the any/all functions in the guard
                guard_expression.parse_any_all(
                    continuous_variable_dict, discrete_variable_dict, length_dict)
                ...
                guard_can_satisfied = guard_expression.evaluate_guard_hybrid(
                    agent, discrete_variable_dict, continuous_variable_dict, self.map)
                if not guard_can_satisfied:
                    continue
    ...
    def apply_reset(self, agent, reset_list, all_agent_state) -> Tuple[str, np.ndarray]:
        ...
        continuous_variable_dict, discrete_variable_dict, _ = self.sensor.sense(
            self, agent, all_agent_state, self.map)
        dest = reset_expr.get_dest(
            agent, all_agent_state[agent.id], discrete_variable_dict, self.map)
        rect = reset_expr.apply_reset_continuous(
            agent, continuous_variable_dict, self.map)
        return dest, rect
                        ...
                        if "mode" in reset:
                            ...
                            if 'map' in tmp[1]:
                                ...
                                for var in discrete_variable_dict:
                                    tmp = tmp.replace(
                                        var, f"'{discrete_variable_dict[var]}'")
                                res = eval(tmp)
    ...
    def get_transition_simulate_new(self, node: AnalysisTreeNode) -> Tuple[Dict[str, List[Tuple[float]]], float]:
        ...
        for agent_id in node.agent:
            ...
            for path in paths:
                ...
                continuous_variable_updater = guard_expression.parse_any_all_new(
                    cont_var_dict_template, discrete_variable_dict, len_dict)
                if agent_id not in agent_guard_dict:
                    agent_guard_dict[agent_id] = [(guard_expression, continuous_variable_updater, copy.deepcopy(
                        discrete_variable_dict), reset_list)]
                else:
                    agent_guard_dict[agent_id].append(
                        (guard_expression, continuous_variable_updater, copy.deepcopy(discrete_variable_dict), reset_list))
        ...
        for idx in range(trace_length):
            ...
            for agent_id in agent_guard_dict:
                ...
                agent_state, agent_mode = state_dict[agent_id]
                agent_state = agent_state[1:]
                continuous_variable_dict, _, _ = self.sensor.sense(
                    self, agent, state_dict, self.map)
                for guard_expression, continuous_variable_updater, discrete_variable_dict, reset_list in agent_guard_dict[agent_id]:
                    ...
                    guard_satisfied = one_step_guard.evaluate_guard(
                        agent, new_cont_var_dict, discrete_variable_dict, self.map)
                    if guard_satisfied:
                        # If the guard can be satisfied, handle resets
                        next_init = agent_state
                        dest = copy.deepcopy(agent_mode)
    ...
    def get_transition_verify_new(self, node: AnalysisTreeNode):
        ...
        for agent_id in node.agent:
            ...
            for path in paths:
                ...
                if not guard_can_satisfied:
                    continue
                if agent_id not in agent_guard_dict:
                    agent_guard_dict[agent_id] = [(guard_expression, cont_var_updater, copy.deepcopy(
                        discrete_variable_dict), reset_list)]
                else:
                    agent_guard_dict[agent_id].append(
                        (guard_expression, cont_var_updater, copy.deepcopy(discrete_variable_dict), reset_list))
        ...
        for idx in range(0, trace_length):
            ...
            for tmp in node.agent:
                state_dict[tmp] = (
                    node.trace[tmp][idx*2:idx*2+2], node.mode[tmp])
